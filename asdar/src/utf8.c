...
{
#define CHECK_UTF8LEN(n)      do{ if (utf8len < (size_t)(n)) { if(interp)errobj_set(interp, &errtype_value, "unexpected end of string");          return -1; }}while(0)
#define CHECK_CONTINUATION(c) do{ if ((c)>>6 != 1<<1)        { if(interp)errobj_set(interp, &errtype_value, "invalid continuation byte %B", (c)); return -1; }}while(0)
	if (uutf8[0] >> 7 == 0) {
		CHECK_UTF8LEN(1);
		*ptr = uutf8[0];
		return 1;
...
bool utf8_validate(Interp *interp, const char *utf8, size_t utf8len)
{
	...
	while (utf8len > 0) {
		...
		uint32_t codepnt;
		if( (nbytes = decode_character(interp, uutf8, utf8len, &codepnt)) == -1 ) return false;
		if( (expected_nbytes = how_many_bytes(interp, codepnt)) == -1 ) return false;
...
bool utf8_decode(Interp *interp, const char *utf8, size_t utf8len, uint32_t **unicode, size_t *unicodelen)
{
	...
	if (!( *unicode = malloc(utf8len*sizeof( (*unicode)[0] )) )) {
		errobj_set_nomem(interp);
		return false;
	}
	...
	*unicodelen = 0;
	while (utf8len > 0) {
		int nbytes = decode_character(interp, uutf8, utf8len, &(*unicode)[*unicodelen]);
		assert(nbytes != -1);
		assert(nbytes == how_many_bytes(interp, (*unicode)[*unicodelen]));
	...
	*unicode = realloc(*unicode, (*unicodelen + 1)*sizeof(uint32_t));
	assert(*unicode);
	return true;
}
