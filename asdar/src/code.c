...
void codeop_debug(const struct CodeOp *op)
{
	...
	#define BOILERPLATE(KIND) case KIND: puts(#KIND); break;
		...
		BOILERPLATE(CODE_SETVAR);
		BOILERPLATE(CODE_GETVAR);
		...
		BOILERPLATE(CODE_SWAP2);
		BOILERPLATE(CODE_THROW);
		BOILERPLATE(CODE_CREATEFUNC);
		BOILERPLATE(CODE_VOIDRETURN);
		BOILERPLATE(CODE_VALUERETURN);
		BOILERPLATE(CODE_DIDNTRETURNERROR);
		...
		BOILERPLATE(CODE_FS_VOIDRETURN);
